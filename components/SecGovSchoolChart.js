...
export default class SecGovSchoolChart extends React.Component {
  ...
  render() {
    ...
    let margin = {top: 250, right: 50, bottom: 300, left: 300}

    if (indic_name==='per_room_board' || indic_name==='per_room_desks' || indic_name==='per_room_sit' || indic_name==='per_washing_nearby' || indic_name==='per_washing_nearby_water' || indic_name==='vip_flush_per') {

        margin = {top: 250, right: 75, bottom: 300, left: 300}
    }

    const width = screen.width - margin.left - margin.right
    const height = screen.height - margin.top - margin.bottom
    const data = this.props.data.values.school.values.filter(function(d) { return (d.school_type==='Government: Secondary' || d.school_type==='Community: Secondary')});
    ...
    return(
      ...
      <Group x={50} y={35}>
          <Text
              ...
              alignment="left"
            >
            Note: "Sec" used in school names is short for "Secondary"
          </Text>
      </Group>
